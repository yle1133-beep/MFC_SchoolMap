...
#include "pch.h"
#include "framework.h"
#include "MAP1.h"         // ★ 본인 프로젝트 이름 헤더 (예: MAP1.h)
#include "ChildView.h"
#include "Resource.h"     // ★ 리소스 ID(IDB_PNG1 등)를 쓰기 위해 필수
...
CChildView::CChildView()
{
    ...
    // ----------------------------------------------------------
    // [이미지 로드 로직] PNG/JPG 호환을 위한 스트림 방식
    // ----------------------------------------------------------

    // ★★★ [중요] 본인의 리소스 ID로 변경하세요 (예: IDB_PNG1, IDR_IMAGE1 등) ★★★
    // ★★★ [중요] 리소스 뷰의 폴더명이 "PNG"가 아니라면 그 이름으로 변경하세요 ("JPG" 등) ★★★
...
void CChildView::OnLButtonDown(UINT nFlags, CPoint point)
{
    ...
    if (clickedIdx != -1) {
        ...
        // GetKeyState(VK_CONTROL) < 0 은 키가 눌린 상태를 의미합니다.
        ...
        // ★★★ 여기가 수정된 핵심 부분입니다 (VK_MENU가 Alt키입니다) ★★★
...
void CChildView::RunDijkstra()
{
    ...
    if (dist[curr] != INF) {
        ...
        // ★★★ [핵심 수정] 팝업 뜨기 전에 화면 먼저 그리기 ★★★
        Invalidate();   // "화면을 다시 그려야 한다"고 요청
        UpdateWindow(); // 대기하지 않고 "즉시" 그리기 실행 (빨간 선 등장)
